...
class HomeCubit extends Cubit<HomeState> {
  HomeCubit() : super(HomeInitial()){
    _authApi.updateUserPresenceInRealtimeDB();
  }

  static HomeCubit get(context) => BlocProvider.of(context);

  final firebaseNotifications = FirebaseNotificationsHandler();

  final _authApi = AuthApi();

  void initFcm(context){
    firebaseNotifications.setUpFcm(context: context, onForegroundClickCallNotify: (String payload){
      printDebug('Foreground Click Call Notify: $payload');
    });
  }
  ...
  void updateFcmToken({required String uId}) {
    FirebaseMessaging.instance.getToken().then((token) {
      ...
      _authApi.updateFcmToken(tokenModel: tokenModel);
    });
  }
  ...
  void getUsersRealTime(){
      ...
      usersStreamSubscription = _homeApi.getUsersRealTime();
      usersStreamSubscription?.onData((data) {
        if(data.size!=0){
    ...
    .then((value) {
      if(value.exists){
        ...
        DioHelper.postData(
          ...
          printDebug('SendNotifySuccess ${value.data.toString()}');
          emit(SuccessFireVideoCallState(callModel: callModel));
        }).catchError((onError){
          printDebug('Error when send Notify: $onError');
          fireCallLoading = false;
          emit(ErrorSendNotification(onError.toString()));
        });
      }
    }).catchError((onError){
      printDebug('Error when get user token: $onError');
      fireCallLoading = false;
      emit(ErrorSendNotification(onError.toString()));
    });
  }
 // CallModel inComingCall;
        ...
        for (var element in data.docs) {
          if(element.data()['current'] == true){
            ...
            if(status == CallStatus.ringing.name){
              ...
              controlUserStreamSubscription(isPause: true);
              printDebug('ringingStatus');
  ...
  controlUserStreamSubscription({required bool isPause}){
    if(isPause){
      usersStreamSubscription?.pause();
    }else{
      usersStreamSubscription?.resume();
    }
  }
}
